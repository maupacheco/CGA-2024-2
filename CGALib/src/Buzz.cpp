...
//jerarquía 
		void Buzz::hierarchy(float rotBuzzBody, float rotBuzzHead, float rotBuzzLeftArm, float rotBuzzLeftForearm, 
			...
			//Ala izquierda 1 (pegada al torso)
...
void Buzz::translate(glm::vec3 distance){ modelMatrixBuzz = glm::translate(modelMatrixBuzz, distance); }
void Buzz::rotate(float angle, glm::vec3 axis){ modelMatrixBuzz = glm::rotate(modelMatrixBuzz, angle, axis); }
void Buzz::scale(glm::vec3 scale){ modelMatrixBuzz = glm::scale(modelMatrixBuzz,scale); }
